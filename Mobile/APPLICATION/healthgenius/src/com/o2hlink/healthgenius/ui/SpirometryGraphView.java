...
public class SpirometryGraphView extends View {
    ...
    @Override protected void onDraw(Canvas canvas) {
		View graph = (View) HealthGenius.myApp.findViewById(R.id.graph);
...
//		float maxLegendXFlow = (float)spirometer.flowLine.length/((float)atfev1)*spirometer.results.get(SensorManager.DATAID_FEV1);
		...
		canvas.drawText(HealthGenius.myLanguageManager.SENSORS_SPIRO_TIMECHAT_LEGENDX, 37, 18, time);
		canvas.drawText(HealthGenius.myLanguageManager.SENSORS_SPIRO_FLOWCHAT_LEGENDX, 37, 28, flow);
		// Draw vertical axis and legend
		canvas.drawLine(30, 20, 30, height - 20, axis);
		canvas.drawText(HealthGenius.myLanguageManager.SENSORS_SPIRO_TIMECHAT_LEGENDY, (float)(width) - 55, height - 50, time);
		canvas.drawText(HealthGenius.myLanguageManager.SENSORS_SPIRO_FLOWCHAT_LEGENDY, (float)(width) - 55, height - 40, flow);
		// Draw vertical marks
