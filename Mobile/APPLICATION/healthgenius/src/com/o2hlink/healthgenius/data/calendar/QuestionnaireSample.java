...
public class QuestionnaireSample extends com.o2hlink.activ8.client.entity.QuestionnaireSample {
	...
	public Spanned getQuestionnaireSpanData(String name) {
		...
		while (elements.hasMoreElements()) {
			Array<Answer> ans = HealthGenius.myQuestControlManager.activeAnswers.get(elements.nextElement().getId());
			if (ans != null) answersPostemp.addAll(ans);
		}
		for (Answer answer : answersPostemp) {
			answerPos.put(answer.getId(), answer);
		}
		String spaning = "<big><b>" + HealthGenius.myLanguageManager.PATIENTS_HISTORY_QUESTIONNAIRE + " " + name + ", " + HealthGeniusUtil.dateToReadableString(this.getDate()) + " " + HealthGeniusUtil.timeToReadableString(this.getDate()) + "</b></big><br/><br/>";
		for (QuestionAnswer answer : this.getAnswers()) {
			if (answer instanceof MultiAnswer) {
				...
				spaning += "<p><b>" + HealthGenius.myQuestControlManager.activeQuestions.get(answer.getQuestion()).getName() + "</b><br/>" + answerText + "</p>";
			}
