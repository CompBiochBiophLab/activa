...
public class PedometerCalibration {
	...
	public void finishCalibration(boolean outcome, float mLimit) {
		...
		HealthGenius.myMobileManager.pedometerCalValue = Math.round(mLimit);
		HealthGenius.myMobileManager.user.pedometerCalValue = Math.round(mLimit);
		...
		text.setText(HealthGenius.myLanguageManager.SENSORS_CALIBRATION_DONE);
		if (mLimit <= 30) text.append(HealthGenius.myLanguageManager.SENSORS_CALIBRATION_SELECT_ULTRAHIGH);
		else if (mLimit <= 35) text.append(HealthGenius.myLanguageManager.SENSORS_CALIBRATION_SELECT_VERYHIGH);
		else if (mLimit <= 40) text.append(HealthGenius.myLanguageManager.SENSORS_CALIBRATION_SELECT_HIGH);
		else if (mLimit <= 45) text.append(HealthGenius.myLanguageManager.SENSORS_CALIBRATION_SELECT_NORMAL);
		else if (mLimit <= 50) text.append(HealthGenius.myLanguageManager.SENSORS_CALIBRATION_SELECT_LOW);
		else if (mLimit <= 55) text.append(HealthGenius.myLanguageManager.SENSORS_CALIBRATION_SELECT_VERYLOW);
		else text.append(HealthGenius.myLanguageManager.SENSORS_CALIBRATION_SELECT_ULTRALOW);
...
//		mLimit = ((mLimit < minDiff) ? mLimit : minDiff);
	...
	private Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			switch (msg.what) {
			case 0:
				HealthGenius.myUIManager.loadScreen(R.layout.follow);
				TextView text = (TextView) HealthGenius.myApp.findViewById(R.id.textInfo);
				((ImageButton) HealthGenius.myApp.findViewById(R.id.follow)).setVisibility(View.GONE);
				text.setTextSize(14);
				text.setText(HealthGenius.myLanguageManager.SENSORS_CALIBRATION_INSTRUCTIONS);
				break;
			case 1: 
				calibrator.finishCalibration(true, mLimit);
				break;
			case 2:
				((Vibrator)HealthGenius.myApp.getSystemService(Context.VIBRATOR_SERVICE)).vibrate(1000);
				ImageButton next = (ImageButton) HealthGenius.myApp.findViewById(R.id.follow);
