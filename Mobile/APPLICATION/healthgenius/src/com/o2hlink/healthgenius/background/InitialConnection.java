...
public class InitialConnection implements Runnable {
	...
	public void run() {	
		...
		if (success) {
		    ...
		    /* Send pending data */
	        HealthGenius.myPendingDataManager = PendingDataManager.getInstance();
		...
		Thread thread = new Thread(HealthGenius.myMainBackgroundThread, "MAIN");
		thread.start();
		...
		public void handleMessage(Message msg) {
			switch (msg.what) {
				case 0:
					...
					break;
				case 100:
					if (!HealthGenius.myMobileManager.user.isCreated()) popupText.setText(HealthGenius.myLanguageManager.CONNECTION_REGISTERING);
					else popupText.setText(HealthGenius.myLanguageManager.CONNECTION_CONNECTING);
					break;
				case 1:
...
class BeginTimeTask extends CountDownTimer {
	...
	public void onFinish() {
		...
		ImageView promotionView = (ImageView) HealthGenius.myApp.findViewById(R.id.popupPromotion);
		if (promotionView != null) promotionView.setVisibility(View.GONE);
		TextView promotion = (TextView) HealthGenius.myApp.findViewById(R.id.popupPromotionText);
		...
		if (HealthGenius.myContactsManager.entryContactList.size() >= 1) HealthGenius.myUIManager.UIcontacts.loadEntryContactList(false);
	}
