...
public class Event {
	...
	public Event() {
		...
		this.subtype = -1l;
		this.state = 1;
		this.userId = Activa.myMobileManager.userForServices.getId();
	}
			...
			switch (this.type) {
				case 0:
					run = new Runnable() {
						@Override
						public void run() {
							if (Activa.myPatientManager.getMeasurementSample(event))
						...
						private Handler handler = new Handler() {
							...
							public void handleMessage(Message msg) {
								...
								switch (msg.what) {
									case 0:
										Activa.myUIManager.loadInfoPopupWithoutExiting(Activa.myLanguageManager.CALENDAR_LOADING);
										...
										break;
									case 1:
										if (subtype.equals(com.o2hlink.activa.data.sensor.SensorManager.ID_PULSIOXYMETER)) {
											Activa.myUIManager.showMeasResults(sample, false);
										}
										else if (subtype.equals(com.o2hlink.activa.data.sensor.SensorManager.ID_SPIROMETER)) {
											Activa.myPatientManager.getSpiroGraphs(userId, sample.getDate(), sample);
											Activa.myUIManager.showMeasResults(sample, false);
										}
										else if (subtype.equals(com.o2hlink.activa.data.sensor.SensorManager.ID_SIXMINUTES)) {
											Activa.myPatientManager.getSixMinutesGraphs(userId, sample.getDate(), sample);
											Activa.myUIManager.showMeasResults(sample, false);
										}	
										else if (subtype.equals(com.o2hlink.activa.data.sensor.SensorManager.ID_WEIGHT)) {
											Activa.myUIManager.showMeasResults(sample, true);
										}
										else if (subtype.equals(com.o2hlink.activa.data.sensor.SensorManager.ID_EXERCISE)) {
											Activa.myUIManager.showMeasResults(sample, true);
										}
										break;
									case 2:
				...
				case 1:
					run = new Runnable() {
						...
						public void run() {
							...
							boolean success = Activa.myPatientManager.getQuestSample(event);
							...
							public void handleMessage(Message msg) {
								...
								switch (msg.what) {
									...
									case 1:
										...
										animation.stop();
										animationFrame.setVisibility(View.GONE);
										Activa.myUIManager.showQuestResults((com.o2hlink.activ8.client.entity.QuestionnaireSample) questsample, false, name);
										break;
									case 2:
										animationFrame = (ImageView) Activa.myApp.findViewById(R.id.popupImage);
										animation = (AnimationDrawable) animationFrame.getBackground();
