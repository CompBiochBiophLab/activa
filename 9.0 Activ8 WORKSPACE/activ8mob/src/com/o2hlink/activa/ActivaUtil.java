...
public final class ActivaUtil
{
    ...
    public static String formatDate(long dateToFormat) {
        return formatDate(dateToFormat, "mm/dd/yyyy");
    }

    public static String formatDate(Calendar dateToFormat) {
        return formatDate(dateToFormat, "mm/dd/yyyy");
    }

    public static String formatDate(long dateToFormat, String formatAs) {
        if(DEBUG){
            System.out.println("MHUtil.formatDate: dateToFormat="+ dateToFormat);
        }
        Date date = new Date(dateToFormat);
        Calendar cal= Calendar.getInstance();
        cal.setTime(date);
        return formatDate(cal, formatAs);
    }
    ...
    /**
     * Formats a date in a readable form
     * 
     * @param dateToFormat The date to be formatted
     * @param formatAs The format to use. Ex: mm/dd/yyyy
     * @return
     */
    public static String formatDate(Calendar cal, String formatAs) {
        if(DEBUG){
            System.out.println("MHUtil.formatDate: dateToFormat=["+ calendarToString(cal) +"] :: long=["+ (cal!=null?cal.getTime().getTime():0) +"]");
        }
        if (cal == null) {
            return null;
        }
        ...
        String sep = "/";
        if (formatAs.indexOf("/") >= 0) {
            sep = "/";
        } else if (formatAs.indexOf("-") >= 0) {
            sep = "-";
        } else {
            return Activa.myLanguageManager.UI_INVALID_FORMAT + formatAs; 
        }
        vec = ActivaUtil.splitText(formatAs.toLowerCase(), sep);
        for (Enumeration enumer = vec.elements(); enumer.hasMoreElements();) {
            String format = (String) enumer.nextElement();
            if (format.startsWith("d")) {
                ret.append(ActivaUtil.leftPadText("" + cal.get(Calendar.DAY_OF_MONTH), '0', format.length()));
            }
            if (format.startsWith("m")) {
                ret.append(ActivaUtil.leftPadText(String.valueOf(cal.get(Calendar.MONTH) + 1), '0', format.length()));
            }
            if (format.startsWith("y")) {
                ret.append(ActivaUtil.leftPadText("" + cal.get(Calendar.YEAR), '0', format.length()));
            }
     ...
     * @return
     */
    public static Vector splitText(String s, String separator) {
        Vector v = new Vector();
    ...
    /**
     * Decode a number from a byte array.
     * 
     * @param bytes
     * @param from 
     ...
     */
	public static long decodeNumber(byte[] bytes, int from, int len, boolean isSigned) throws InvalidValueException {
	    if(bytes==null){
	        //"'bytes' shall be not null"	        
	        throw new InvalidValueException(Activa.myLanguageManager.UI_DEFAULT_ERROR_MSG);
	    }
	    if(from<0){
	        //"'from' shall be >= 0"
	        throw new InvalidValueException(Activa.myLanguageManager.UI_DEFAULT_ERROR_MSG);
	    ...
	    if(from+len > bytes.length){
	        //" 'from+len' [" + (from + len) + "] shall be <= 'bytes.length' [" + bytes.length + "]"
	        throw new InvalidValueException(Activa.myLanguageManager.UI_DEFAULT_ERROR_MSG);
	    }
		long val = 0;
		int where = 0;
		if ((isSigned) && (bytes[from] < 0)) {
			val = ~0;
		} else {
			val = 0;
		}
		while (where < len) {
			val = (val << 8) | (bytes[from + where] & 0xFF);
			where++;
		}
		return val;
	}
	...
	public static final String ENC_UCS2 = "UCS2";
	public static final String ENC_GBK = "GBK";
	public static final String ENC_OCTET = "octet";
	 ...
	 * @param enc How to encode the String. See ENC_ASCII, ENC_UCS2, ENC_GBK, ENC_OCTET
	 * @return
	 * @throws InvalidValueException
	 */
	public static String decodeString(byte[] bytes, int from, int len, java.lang.String enc) throws InvalidValueException {
	    if(bytes==null){
	        //"'bytes' shall be not null"
	        throw new InvalidValueException(Activa.myLanguageManager.UI_DEFAULT_ERROR_MSG);
	    }
		...
		/* decode a string and put it to the hash table with its name */
		String s = new String();
		int l = len - 1;
		if (0 == enc.compareTo(ENC_ASCII)) {
			enc = "US-ASCII";
		} else if (0 == enc.compareTo(ENC_UCS2)) {
			enc = "ISO-10646-UCS-2";
		} else if (0 == enc.compareTo(ENC_GBK)) {
			enc = "GBK";
		} else if (0 == enc.compareTo(ENC_OCTET)) {
			enc = null;
		} else {
			enc = null;
		}
		if (null != enc) {
			/* ASCII, UCS2(big endian) and GBK do not have characters end with byte 0 */
			while (bytes[from + l] == 0) {
				if (l < 0) {
					break;
				}
				l--;
			}
			s = new java.lang.String(bytes, from, l + 1);
		}
		return s;
	}
	...
	public static String monthOfDate (Date date) {
		...
		switch (date.getMonth() + 1) {
			case 1:
				returned = Activa.myLanguageManager.MONTH_JANUARY;
				break;
			case 2:
				returned = Activa.myLanguageManager.MONTH_FEBRUARY;
				break;
			case 3:
				returned = Activa.myLanguageManager.MONTH_MARCH;
				break;
			case 4:
				returned = Activa.myLanguageManager.MONTH_APRIL;
				break;
			case 5:
				returned = Activa.myLanguageManager.MONTH_MAY;
				break;
			case 6:
				returned = Activa.myLanguageManager.MONTH_JUNE;
				break;
			case 7:
				returned = Activa.myLanguageManager.MONTH_JULY;
				break;
			case 8:
				returned = Activa.myLanguageManager.MONTH_AUGUST;
				break;
			case 9:
				returned = Activa.myLanguageManager.MONTH_SEPTEMBER;
				break;
			case 10:
				returned = Activa.myLanguageManager.MONTH_OCTOBER;
				break;
			case 11:
				returned = Activa.myLanguageManager.MONTH_NOVEMBER;
				break;
			case 12:
				returned = Activa.myLanguageManager.MONTH_DECEMBER;
				break;
		}
		return returned;
	}
